...
def graph_reader(graph_filename: str) -> Graph:
    ...
    for line in lines:
        if line[0] == "c":
            ...
            graph.edges.append(sorted([int(x) for x in line.strip("e").split()]))
            # check if duplicates
    return graph


def produce_cnf(problemType: str, pattern_filename: str, graph_filename: str) -> str:
    # read input files
    ...
    if problemType == "--emb":
        for v in range(1, graph.number_of_verticies + 1):
            for a in range(1, pattern.number_of_verticies):
                for b in range(a + 1, pattern.number_of_verticies):
                    cnf.clauses.append([-pair(a, v), pair(b, v)])

    cnf.number_of_clauses = len(cnf.clauses)
    cnf_file_name = "generated-input/cnf-{}-h-{}-g-{}.cnf".format(problemType.strip("--"), pattern_filename.split("/")[-1], graph_filename.split("/")[-1])
    write_cnf_file(cnf, cnf_file_name)
