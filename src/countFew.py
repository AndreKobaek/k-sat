...
def count_few_branching(F: CNF, maximum_runs: int = sys.maxsize):
    ...
    if satisfiable():
        ...
        if F.number_of_literals != 0 and F.number_of_clauses !=0 and [] not in F.clauses:
            number_of_solutions = count_few_branching(set_var(
                F,  1), maximum_runs) + count_few_branching(set_var(F, -1), maximum_runs)
...
#print(set_up_branching("input/simpleCNF.txt",10))