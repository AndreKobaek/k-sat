...
//x(a) <=> a+b
// final solution by solving x(a) = a+b
void SameReplaceSolve(int *Fsolution, PPS *pps)
{  
    if(pps->sameReplace==0) return;
    if(pps->sameReplace->nowpos()) printf("c SameReplaceSolve \n");
    while(pps->sameReplace->nowpos()){
      int type  =pps->sameReplace->pop();
      int maxLit=pps->sameReplace->pop();
      int minVar=pps->sameReplace->pop();
      if(type==ALIAS_TYPE){//x= a (+) b
             if(Fsolution[minVar]>0){
                   Fsolution[minVar]=Fsolution[maxLit]>0? -minVar:minVar;
             }
             else  Fsolution[minVar]=Fsolution[maxLit]>0? minVar :-minVar;
      }
      if(type==EQV_TYPE){//a=b
              int maxVar=ABS(maxLit);
              Fsolution[minVar]=Fsolution[maxVar]>0? minVar : -minVar;
              if(maxLit < 0) Fsolution[minVar] = -Fsolution[minVar];
      }    
   }
}   

// let a=minVar b=maxVar,   x <=> a (+) b   let x=a 
void gaussSameReplace(PPS *pps, int minVar, int maxVar)
{  
   Stack<int> clause;
   Stack<int> * clsSAT=pps->clause;
   if(pps->sameReplace==0) pps->sameReplace = new Stack<int>;
   minVar--;
   maxVar--;
  // printf("\n Replace minvar=%d maxvar=%d:",minVar,maxVar);
   pps->sameReplace->push(minVar);
   pps->sameReplace->push(maxVar);
   pps->sameReplace->push(ALIAS_TYPE);

   Stack<int> loc[2];
   for(int i=0; i<2; i++){
       int lit=(i==0)? minVar:-minVar;
       while(pps->occCls[lit].nowpos()) loc[i].push(pps->occCls[lit].pop());
   }
   for(int i=0; i<2; i++){
      int lit=(i==0)? minVar:-minVar;
      for(int k=0; k<loc[i].nowpos(); k++){
         int pos=loc[i][k];
         int len=(*clsSAT)[pos];
	 len=len>>FLAGBIT;
         (*clsSAT)[pos]=(len<<FLAGBIT) | DELETED;
         pps->garbage += len;
...
//         printf("\nlen=%d lit=%d ",len,lit);
         ...
         for (int k=pos+1; k<pos+len; k++){
                int curlit=(*clsSAT)[k];
     //           printf(" %d ",curlit);
                if(lit==curlit) continue;
                pps->occCls[curlit].remove(pos);//rem index
                if(ABS(curlit)==maxVar){
                       maxlit=curlit;  
                       continue;
                }
                clause.push(curlit);
         }
         int newlit=lit<0? maxVar : -maxVar; // x<=> a+b
         if(newlit != -maxlit) addclause(pps, clsSAT, clause, newlit, -minVar);
         newlit=-newlit;
         if(newlit != -maxlit) addclause(pps, clsSAT, clause, newlit, minVar);
      }
   }
...
{  
   ...
   Stack<int> * clsSAT=pps->clause;
   if(pps->sameReplace==0) pps->sameReplace = new Stack<int>;
   minLit = minLit < 0? minLit+1 : minLit-1;
   maxLit = maxLit < 0? maxLit+1 : maxLit-1;

  // printf("\n Eqv Replace minLit=%d maxLit=%d:",minLit,maxLit);

   pps->sameReplace->push(minLit);
   pps->sameReplace->push(maxLit);
   pps->sameReplace->push(EQV_TYPE);

   for(int i=0; i<2; i++){
      minLit = -minLit;
      maxLit = -maxLit;
      while(pps->occCls[minLit].nowpos()){
         int pos=pps->occCls[minLit].pop();
         int len=(*clsSAT)[pos];
	 len=len>>FLAGBIT;
         int remove=0;
         for (int k=pos+1; k<pos+len; k++){
                int curlit=(*clsSAT)[k];
                if(curlit==-maxLit){ remove=1; break;}
                if(curlit==maxLit) { remove=2; break;}
         }
         clause.shrink(0);
        // printf("\nlen=%d minLit=%d ",len,minLit);
         for (int k=pos+1; k<pos+len; k++){
                int curlit=(*clsSAT)[k];
           //     printf(" %d ",curlit);
                if(minLit==curlit){
                       if(remove==0) (*clsSAT)[k]=maxLit;
                       continue;
                }
                if(remove){
                        pps->occCls[curlit].remove(pos);
                        if(remove==2) clause.push(curlit);
                }
         }
         if(remove){
            pps->garbage += len;
         ...
         switch(remove){
             case 0: //printf("\n");
                     //for (int k=pos+1; k<pos+len; k++) printf(" %d ",(*clsSAT)[k]);
                     pps->occCls[maxLit].push(pos);
                     break; 
             case 1: //printf(" deleted \n");
                     break;
             case 2: addclause2(pps, clsSAT,clause);break;
         }
      }
   }
}
